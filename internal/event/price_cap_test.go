...
func TestProposalPriceCapMatchesItsDerivation(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "a zero cap on a stop-limit",
			entry:   caps{event.OrderTypeStopLimit, 1, 0},
			add:     caps{event.OrderTypeStopLimit, 1, 0},
			wantErr: "price cap must be positive"},
		{name: "a cap below the level",
			entry:   caps{event.OrderTypeStopLimit, 0, entryLevel - 1},
			add:     caps{event.OrderTypeStopLimit, 0, addLevel - 1},
			wantErr: "below the level"},

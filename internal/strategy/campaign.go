...
func (c *campaignState) entryPrice() float64 {
	...
	var quantity int64
	for _, u := range c.units {
		weighted += float64(float64(u.quantity) * u.fillPrice)
		quantity += u.quantity
	}
...
func (r *Reducer) evaluateAdd(state *instrumentState, input event.Envelope) ([]event.Envelope, error) {
	...
	// SAME rung and quantity a successful proposal would have.
	availableCash, err := r.cashAtPreviousClose(campaign.instrumentID, state.lastBarEarliestFillAt)
	if err != nil {
		return nil, err
	}
...
func (r *Reducer) applyStopFill(state *instrumentState, fill event.FillPayload, input event.Envelope) ([]event.Envelope, error) {
	...
	for i, u := range closingUnits {
		thisEntryWeightedSum += float64(float64(u.quantity) * u.fillPrice)
	...
	campaign.closedExitWeightedSum += float64(float64(closingQuantity) * fill.Price)

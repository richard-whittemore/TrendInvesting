...
**That overshoot is bounded in turn, at `journal.MaxEmissionsPerInput` (65,536).** An input causing more decisions than that is refused whole — nothing of it is recorded — so a run holds at most `maxRecords + MaxEmissionsPerInput` records rather than an amount the handler chooses. The ceiling is needed because the largest burst one input can cause scales with the universe and not with the bound: ending the input stream expires every proposal still outstanding, which is ADR 0011's three proposal kinds across every instrument the run holds state for, while every other input decides for one instrument. 65,536 covers a universe two orders of magnitude larger than the hundred instruments sized below, and caps the overshoot at about a thirtieth of the bound itself.

Two million records is roughly twice the largest daily-bar run this platform is built for — a decade over a hundred instruments is about a million, at the golden fixture's ratio of records to bars — and about two gigabytes of resident entries at that fixture's ~1 kB per record. Both figures are arithmetic rather than measurement, and the per-record size is the shakier of the two: ~1 kB is the mean *serialised* line length of the golden journal's records, which stands in for the resident size of an `Entry` without having measured one. It is a tripwire, not a capacity model: its job is to fire *before* the allocator does, so the failure is a diagnosable refusal instead of a process killed with nothing on disk.

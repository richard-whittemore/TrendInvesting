...
6. at the 00:01 pre-session check, when LEAN has split its open orders, amends each reduced Unit's Exit Order to the engine's new quantity. LEAN's own split of an order is rounded by the same factor, so any working order LEAN left within one raw share of the engine's figure at the new ratio is amended to exactly that figure, reductions before increases so that working sells never exceed the holding. The existing split checks then run unchanged. A difference of more than one raw share on any order, or any other difference, stops the run exactly as before.

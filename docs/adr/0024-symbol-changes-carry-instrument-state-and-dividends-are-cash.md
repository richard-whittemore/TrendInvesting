...
## Amendment (2026-09-25): corporate-action delivery is global, not per instrument

A third review pass raised four more findings, all in `cmd/backtest`'s delivery of corporate actions ahead of the bars they must precede (§4 of the Decision, above), all against the SAME underlying design: `deliverActionsDueFor` (since removed) decided whether an action was "due" by checking it against ONE instrument's own next bar.

1. `symbolChangeChain`'s backward walk, added for the second amendment's finding 2, had no visited check: `A -> B` plus `B -> A` walked forever.
2. A dividend on a symbol-change chain's own INTERMEDIATE id — one that never receives a bar of its own — could not be found by any per-instrument lookup at all, symbol-change chain or not, since the lookup only ever asked "what names THIS bar's own instrument". Swept into `deliverRemainingActions`, it was refused as terminal even though the chain's destination went on to receive a bar afterward.
3. Two renames sharing one exact instant (`Z -> A` and `A -> B`) depend on each other for delivery order in a way no single instrument's own boundary check expresses at all; sorted by instrument id alone, the wrong one could go first.

Three different patches — one per finding — would each have fixed one shape and very plausibly left the next, exactly as the second amendment's own two symptoms (a lost Campaign, then a wrongly refused chain) were each a genuine fix for its own finding and neither anticipated the other. Instead of a fourth patch, the whole design changed.

### The new design

Corporate-action delivery is no longer scoped to one instrument's own next bar. `deliverActionsDueBefore` delivers every not-yet-delivered action, across every instrument at once, whose `EffectiveAt` precedes the Session about to open — checked ONCE per Session (every bar in a Session shares one period end, `sessionsOf`), not once per bar. This is a strict generalisation for any instrument that DOES have a bar in that Session: "before this Session's boundary" and "before this bar's own PeriodEnd" are the identical instant whenever the two coincide, which is every existing delisting and split fixture below this ADR. What changes is only the case none of them exercise: an action on an instrument the current Session's bars do not include — which is exactly the shape every one of the three findings above turned out to be.

`effectiveOrder` (replacing `inEffectiveOrder`) sorts the due actions by `EffectiveAt` first, as always, then resolves each EXACT-instant group with `dependencyOrder`: a topological sort (Kahn's algorithm) in which a symbol change producing instrument id `x` is a predecessor of any other action in the same instant naming `x` as its own `InstrumentID`. Ties with no dependency edge keep the original tiebreak — instrument id, then kind, then the whole payload's own encoding — so a fixture with no same-instant chain sorts exactly as it always did. Only a symbol change can ever be a predecessor (no other kind has a `NewInstrumentID`), so only symbol changes can ever form a cycle; Kahn's algorithm terminates in at most `len(group)` steps regardless of the graph, so it cannot loop the way the old backward walk did, and a cycle is refused by name instead — group 1's finding, closed by construction rather than by a visited set bolted onto the old walk.

Dependency ordering is deliberately scoped to actions sharing one EXACT instant. Different-time actions never need it: a later action naming an id an earlier one produced or retired is already delivered in the right relative order by `EffectiveAt` alone, and whether the REDUCER then accepts or refuses it against `Reducer.renamed` (§2, above) is that package's own question, never this sort's to pre-empt.

`symbolChangeChain` is removed entirely: the global sweep makes it unnecessary rather than merely safer, since an intermediate chain id no longer needs its own backward lookup to be found — it is simply due, like everything else, the moment its own `EffectiveAt` is reached.

### Why this still holds ADR 0010/0021's ordering, and the existing delisting semantics

An action is still delivered no earlier than the first Session boundary that exceeds its own `EffectiveAt`, so it still reaches the reducer strictly between Sessions, never mid-Session, and still precedes every bar its own effective time requires it to (ADR 0010/0021). The reducer's own per-instrument chronology (an action must not predate ITS OWN instrument's last completed bar) is unaffected by checking globally: that instrument's last bar, if it has had one, necessarily fell in some Session strictly before the one whose boundary now delivers the action, because Sessions are processed in ascending order and delivery for Session *S* happens before *S*'s own bars are ever read. Delivering "as soon as globally due" can only ever be EARLIER than the old "as soon as due for this instrument's own next bar", never later, and earlier delivery of a fact that has already truly taken effect is not a look-ahead — it is this command finally applying it at the first moment it validly can, rather than waiting on an accident of which instrument's bar happened to come next. A delisting keeps its existing semantics exactly: `deliverActionsDueBefore` still checks `EffectiveAt` before the Session boundary and nothing else, the reducer's own "no Campaign to close" and "already delisted" no-ops are untouched, and every `corporate_actions_delisting*.json` fixture's own instrument has a bar in the very Session its own delisting precedes, so its delivery point does not move at all.

### Goldens

No golden journal or decision-corpus entry changed (`git status`/`git diff --stat` against `cmd/backtest/testdata/` and `internal/strategy/testdata/` are both empty for this amendment): every existing fixture's actions name an instrument with a bar in the Session the action is due before, so `deliverActionsDueBefore` delivers at the exact same boundary `deliverActionsDueFor` did, in the exact same relative order `inEffectiveOrder` always produced (no existing fixture states two actions at one shared instant with a dependency edge between them).

### Tests

`cmd/backtest/symbol_change_chain_test.go` gained `TestASymbolChangeCycleIsRefusedRatherThanHanging` (finding 1: refused, and provably cannot hang, since Kahn's algorithm terminates regardless of the graph), `TestADividendOnAnIntermediateChainInstrumentIsDeliveredBeforeTheDestinationsBar` (finding 2), and `TestEqualTimeRenamesAreDeliveredInDependencyOrder` (finding 3). `TestASymbolChangeChainIsFullyDeliveredBeforeTheDestinationsBar` (the second amendment's own regression test) and `TestCorporateActionsAreDeliveredInATotalOrder` (`cmd/backtest/split_test.go`, updated only to call `effectiveOrder` in place of the removed `inEffectiveOrder`) both continue to pass unchanged, confirming the new design subsumes the old one rather than merely patching around it.

No RulesVersion bump, no corpus change: this amendment touches only `cmd/backtest`'s corporate-action delivery, outside `internal/strategy`'s own decision surface, and changes no reducer decision for any existing fixture.

...
- `-max-records` is the most records the run may hold in memory before its journal is written, defaulting to 2,000,000 — roughly twice a decade of daily bars over a hundred instruments, and about a gigabyte of records. A journal is composed in memory, because the header states the span the run covered and that is not known until the last input has arrived (ADR 0017); the bound is what turns "the process died having written nothing" into a run that stops, says so, and leaves the records it took. A run that reaches it exits non-zero, writes the journal of what it recorded under the span that journal actually covers, and is registered as a failed run. Raise it only knowing the cost is memory in proportion.

...
- `-available-cash` optionally states the USD cash figure on the run's opening `account.snapshot`, independently of the configuration's starting equity. It must be finite and nonnegative; `0` explicitly means no available cash. When it is omitted, available cash equals starting equity. The snapshot records the chosen figure and the first bar's period end as its `as_of`; ADR 0010's previous-close check remains in the reducer. A scalar flag is sufficient for this opening input and needs no new fixture or event schema. The reducer debits every entry and Add fill from this figure at its actual cost (ADR 0020), so the default run takes as many Units as starting equity funds and declines the rest with `insufficient-cash`: with the committed fixture, Unit 1 and none after it. The command states no later snapshot, so it does **not** model daily cash changes, reserve orders, or credit exits, and exit proceeds never return within a run; ADR 0020's daily snapshots and order reservations remain separate work.
- `-corporate-actions` is optional: a JSON array of `event.CorporateActionPayload` (today, only a Delisting Exit — CONTEXT.md: "Delisting Exit"), in any order — where an action lands depends on its own `effective_at` and its own instrument's bars, never on its position in the file, so nothing here demands the fixture be sorted. An empty array (`[]`) declares a run with none, deliberately; a file holding JSON `null` is refused rather than treated the same way; a run given no `-corporate-actions` at all behaves exactly as one always has. Each action is interleaved among the bars **per instrument**: it is delivered before the first bar decision of its OWN instrument that its effective time precedes, never measured against a bar of a different instrument, since `-bars` promises only "the order the run delivers them" and not global chronology across instruments. An action with no later bar of its own to precede — one effective at or after that instrument's final bar, or naming an instrument the fixture has no bars for at all — is delivered after every bar, immediately before `replay.run.completed`. That is the only placement that respects its instrument's own chronology. Whenever more than one action is delivered at the same point — several due before one bar, or several flushed at the end — they go in effective-time order, and where that ties, by instrument and then kind — never in the order the file listed them. Those three fields are the whole of a corporate action, so two the ordering cannot separate are the same value and nothing can observe which went first. Which one the reducer sees is therefore a fact about the instruments rather than about the fixture. It is also why a journal's input records are in **delivery order, not effective-time order**: such an action can carry an effective time earlier than inputs already recorded. This is the same convention fills already follow, where "the order of fills within a bar is carried by the order they are delivered in, not by their timestamps" (`docs/architecture.md`). The journal's span is unaffected, since it is checked against the first and last input record rather than against the earliest and latest time among them.
  Whether a given action is stale relative to what the reducer has already accepted for its instrument is judged entirely by the reducer's own chronology check (`internal/strategy/delisting.go`'s `applyDelisting`), not by this command.
...
The run's inputs are the configuration event, the opening account snapshot, each completed bar (each one driven through the per-bar fill protocol of ADR 0005, so the fills the simulator decides are interleaved as inputs of their own), any supplied corporate actions, and finally an end-of-stream event that expires any proposal still outstanding — so every proposal in a completed run reaches exactly one terminal event.

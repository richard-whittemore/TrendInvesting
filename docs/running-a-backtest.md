...
- `-corporate-actions` is optional: a JSON array of `event.CorporateActionPayload` (today, only a Delisting Exit — CONTEXT.md: "Delisting Exit"), in non-decreasing `effective_at` order **per instrument** (a fixture naming one instrument out of its own order is refused, naming both entries; two actions for *different* instruments have no order between them, since each is placed against its own instrument's bars, and a fixture listing a later one first is well formed). An empty array (`[]`) declares a run with none, deliberately; a file holding JSON `null` is refused rather than treated the same way; a run given no `-corporate-actions` at all behaves exactly as one always has. Each action is interleaved among the bars **per instrument**: it is delivered before the first bar decision of its OWN instrument that its effective time precedes, never measured against a bar of a different instrument, since `-bars` promises only "the order the run delivers them" and not global chronology across instruments. An action with no later bar of its own to precede — one effective at or after that instrument's final bar, or naming an instrument the fixture has no bars for at all — is delivered after every bar, immediately before `replay.run.completed`. That is the only placement that respects its instrument's own chronology, and it is why a journal's input records are in **delivery order, not effective-time order**: such an action can carry an effective time earlier than inputs already recorded. This is the same convention fills already follow, where "the order of fills within a bar is carried by the order they are delivered in, not by their timestamps" (`docs/architecture.md`). The journal's span is unaffected, since it is checked against the first and last input record rather than against the earliest and latest time among them.

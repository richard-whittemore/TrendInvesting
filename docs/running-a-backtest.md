...
- `-available-cash` optionally states the USD cash figure on the run's opening `account.snapshot`, independently of the configuration's starting equity. It must be finite and nonnegative; `0` explicitly means no available cash. Omission preserves the existing assumption that available cash equals starting equity, including the existing journal bytes. The snapshot records the chosen figure and the first bar's period end as its `as_of`; ADR 0010's previous-close check remains in the reducer. A scalar flag is sufficient for this opening input and needs no new fixture or event schema. It does **not** model daily cash changes, debit fills, reserve orders, or credit exits: ADR 0020's daily snapshots and within-bar ledger remain separate work. The recorded opening figure continues to be used throughout the run.

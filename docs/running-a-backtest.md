...
- `-corporate-actions` is optional: a JSON array of `event.CorporateActionPayload` (today, only a Delisting Exit — CONTEXT.md: "Delisting Exit"), in non-decreasing `effective_at` order (a fixture out of order is refused, naming the two entries out of sequence). An empty array (`[]`) declares a run with none, deliberately; a file holding JSON `null` is refused rather than treated the same way; a run given no `-corporate-actions` at all behaves exactly as one always has. Each action is interleaved among the bars **per instrument**: it is delivered before the first bar decision of its OWN instrument that its effective time precedes, never measured against a bar of a different instrument, since `-bars` promises only "the order the run delivers them" and not global chronology across instruments. Whether a given action is stale relative to what the reducer has already accepted for its instrument is judged entirely by the reducer's own chronology check (`internal/strategy/delisting.go`'s `applyDelisting`), not by this command.

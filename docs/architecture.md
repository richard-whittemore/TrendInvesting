...
`fills.RunSession` is the loop, and is the single place the protocol is defined; `#19`'s backtest command and `#30`'s LEAN adapter both follow it rather than re-deriving it. It runs one **Session** at a time (CONTEXT.md; ADR 0021): every completed bar sharing one period end. Steps 1 and 2 run for each bar of the Session in the order given; then `market.session.closed` ends the Session, naming every bar it held, and the reducer decides the Session's Adds and then its entries, across instruments; then step 3 runs for each instrument in ascending instrument order. `fills.RunBar` is a Session of one bar. For each completed bar **B** of an instrument, in order:
...
2. **The bar, then the Session's close.** B is delivered to the reducer, which expires yesterday's proposals (ADR 0011; a fill-chained Add alone survives one more bar while its Campaign is open and that bar proposes no exit) and evaluates an open Campaign — the stop and Exit Channel levels in force, and on a breach the exit proposal — or, with no Campaign open, evaluates the Setup and signals a breakout. No Add or entry is proposed at the bar. When the Session closes, the reducer proposes every Add its bars reached, then sizes every Signal into a trade proposal in ranked order: ADR 0010's "exits before Adds before entries", across the whole universe (ADR 0021).
3. **Intrabar fixpoint.** Every order now resting is evaluated against B, repeatedly, until nothing more fills: every covered **buy** first, then, when no buy is covered, the next covered **sell**: a Unit's Exit Order resting at its own stop, worst price first, and once none is left, the Exit Orders resting at the Exit Channel, as one exit fill. Each fill is delivered as it is decided and whatever it causes — a Unit, its own Exit Order, the next rung, a cancelled Add, a closed Campaign — is folded back into the book before the next pass. This is what lets all four Units be added inside one bar, each rung measured from the fill before it.
4. Anything still unfilled keeps resting. The reducer expires an entry or an ordinary Add at B+1. A fill-chained Add (one proposed in reply to a fill) is expired at B+2 instead, or earlier, at B+1, if that bar proposes an exit. A stop fill that closes the Campaign, in part or in full, cancels any pending Add at once (ADR 0011, as amended).
...
- **Same-bar ambiguity resolves pessimistically**: buys fill before sells, so a bar covering both an entry and the stop that entry sets is entered and *then* stopped, and a bar reaching both a rung and a stop adds the Unit and *then* stops the Campaign. Sells never compete: each held Unit rests exactly one, its **Exit Order**, at the higher of its Protective Stop and any proposed Exit-Channel exit's level (ADR 0005, as amended; the level is the reducer's `strategy.exit-order.set`), so a stop and an exit never both sell the same shares and every covered Exit Order fills at its own level. A Unit at its own stop fills as a stop naming that Unit; the Units at the Exit Channel fill together as the exit, after any stop fills in the same bar, because an exit fill closes everything the Campaign still holds. Knowledge overrides pessimism: an order that gapped through its level executed at the open, which is a fact the bar states rather than an ambiguity to resolve — which is also why an order created by a fill *inside* the bar takes that fill's price, not the bar's open, as its gap reference.

...
4. Anything still unfilled keeps resting. The reducer expires an entry or an ordinary Add at B+1. A fill-chained Add (one proposed in reply to a fill) is expired at B+2 instead, or earlier, at B+1, if that bar proposes an exit or its Campaign has already closed (ADR 0011, as amended).

...
func TestRerunGoldens(t *testing.T) {
	for _, path := range []string{"testdata/variants/recompute-n-at-add/journal.golden.jsonl", goldenJournal, "testdata/variants/profit-protecting-stop/journal.golden.jsonl", "testdata/variants/uncapped/journal.golden.jsonl"} {
		var out bytes.Buffer
